...
import com.kiongroup.dc.function.core.exception.JwtVerificationException;
import com.kiongroup.dc.function.core.exception.ParseBearerTokenException;
import com.kiongroup.dc.function.core.model.AzureTenantLoginKey;
...
public interface AuthenticationMixin extends AzureTenantKeysMixin {
	...
	default Boolean verifyJwtSignature(final SignedJWT signedJWT) throws JwtVerificationException {
		try {
			AzureTenantLoginKey x509Cert = this.getAzureTenantKeys().getKeys().stream().filter(
					azureTenantLoginKey -> azureTenantLoginKey.getX5t().equals(signedJWT.getHeader().getKeyID()))
					.findFirst().orElseThrow(AzureTenantKeysUnavailableException::new);
	...
	default RSAPublicKey getRsaPublicKey(final String x509Cert) throws CertificateException {
		...
		X509Certificate cert = (X509Certificate) factory
				.generateCertificate(new ByteArrayInputStream(DatatypeConverter.parseBase64Binary(x509Cert)));
